...
public class Main extends JFrame {
  ...
  public void drawSplineCurve() {
    ...
    double[] knots = new double[]{0.0, 0.0, 0.0, 0.01, 0.77, 0.78, 1.0, 1.0, 1.0};
    ...
    for (double t = 0.01 ; t <= 0.77 ; t += 0.001) {
      Point p1 = spline.evaluate(t);
    ...
    //for (int n = 0 ; n < evaluatePoints.size() - 1 ; n ++) {//
      //drawLine(evaluatePoints.get(n), evaluatePoints.get(n+1), Color.GREEN);//
    //}//
   ...
   * @param _p2 終点
   */
  public void drawLine(Point _p1, Point _p2,Color c) {
    Graphics2D g = (Graphics2D)m_canvas.getGraphics();
    g.setColor(c);
